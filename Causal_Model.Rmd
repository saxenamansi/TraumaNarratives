...
append_to_csv <- function(new_row) {
  ...
  if (file.exists("Z_Score_Binary.csv")) {
    data <- read_delim("Z_Score_Binary.csv", delim = "|", col_names = TRUE)
    data <- bind_rows(data, new_row)
    write_delim(data, "Z_Score_Binary.csv", delim = "|", append = F)
  } else {
...
fit_causal_model <- function(data, treatment, response, confounder) {
  ...
  formula <- as.formula(paste(response, "~", treatment, "+", paste(confounder, collapse = " + ")))
  model <- glm(formula, data = data, family = binomial)
  sum_mat <- (summary(model)$coefficients)[2, , drop = F]
